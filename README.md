# KmsChain CLI

KmsChain CLI is a tool which enables you to work with KmsChain Crypto API functionality. 
KMSChain Crypto is an open-source high-level cryptographic library that allows you to perform proxy re-encryption as well other necessary operations for secure storing and transferring data in your decentralized app (Blockchain, IoT, etc...)  

```
$ kmschain --help

  KmsChain CLI is a tool which enables you to work with KmsChain Crypto API functionality.
  
  Usage:
    kmschain [command]
  
  Available Commands:
    help        Help about any command
    keys        A brief description of your command
  
  Flags:
    -h, --help   help for kmschain
```
  
```
$ kmschain keys --help
  A brief description of keys command
  
  Usage:
    kmschain keys [flags]
    kmschain keys [command]
  
  Available Commands:
    capsulate   A brief description of key capsulate command
    generate    A brief description of key generate command
  
  Flags:
    -h, --help            help for keys
    -o, --output string   output file
    -p, --pubKey string   publicKey file path
    -s, --secKey string   secretKey file path
```


```
$ kmschain keys generate --help
  A brief description of key generate command
  
  Usage:
    kmschain keys generate [flags]
  
  Flags:
    -h, --help           help for generate
        --publicKey      get public key
        --reEncryptKey   generate reEncryption key
  
  Global Flags:
    -o, --output string   output file
    -p, --pubKey string   publicKey file path
    -s, --secKey string   secretKey file path
```


  Examples
  

  ### generate private key
  ```
  $ kmschain keys generate 
  
    -----BEGIN PRIVATE KEY-----
    6db79c2689f688c3bbdf0ccdcabe6c3887429b9938fca3a0572b9efb127aaaa6
    ------END PRIVATE KEY------
```


  ### get public key from private key
  ```
  $ ./kmschain keys generate --publicKey -s {YOUR_PRIVATE_KEY_FILE_PATH}
  
    -----BEGIN PUBLIC KEY-----
  303232433746334536463833394146373443354531433742453546333831374641394537323339354146453646343937363536373544423532323346334442423343
    ------END PUBLIC KEY------
 ```
 
  
  ### generate reEncryption key
  ```
  $ kmschain keys generate --reEncryptKey -s {YOUR_PRIVATE_KEY_FILE_PATH} -p {YOUR_PUBLIC_KEY_FILE_PATH}
  
    -----BEGIN REENCRYPTION KEY-----
    eb897d2bdcb6123b6fdbd6676f6cf22f4a4bf80782c58f69303650665424cbf4
    ------END REENCRYPTION KEY------
 ```
  
  
```
$ kmschain keys capsulate --help
  brief description of key capsulate command
  
  Usage:
    kmschain keys capsulate [flags]
  
  Flags:
    -c, --capsule string    capsule path
    -d, --decapsulate       decapsulate key
    -e, --encapsulate       encapsulate key
    -h, --help              help for capsulate
    -r, --reEncKey string   reEncryption key path
        --reEncrypt         reEncrypt key
  
  Global Flags:
    -o, --output string   output file (default "pipe:1")
    -p, --pubKey string   publicKey file path
    -s, --secKey string   secretKey file path
```

  ### Encapsulate
  ```
  $ kmschain keys capsulate -e -p {YOUR_PUBLIC_KEY_FILE_PATH}
  
    -----BEGIN CAPSULE-----    
   
    ------END CAPSULE------
    -----BEGIN SYMMETRIC KEY-----

    ------END SYMMETRIC KEY------
```    
  
  ### Decapsulate
  ```
  $ ./kmschain keys capsulate -d -s {YOUR_PRIVATE_KEY_FILE_PATH} -c {YOUR_CAPSULE_FILE_PATH}
  
    -----BEGIN SYMMETRIC KEY-----

    ------END SYMMETRIC KEY------
```

### ReEncrypt
```
$ kmschain keys capsulate --re-encrypt -c {YOUR_CAPSULE_FILE_PATH} --re-enc-key {YOUR_REENCRYPTION_KEY_FILE_PATH} 

    -----BEGIN CAPSULE-----    
   
    ------END CAPSULE------
